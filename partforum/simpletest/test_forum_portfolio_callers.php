 ...
 * is due to change.
 */
Mock::generate('partforum_portfolio_caller', 'mock_caller');
Mock::generate('portfolio_exporter', 'mock_exporter');

class testForumPortfolioCallers extends portfoliolib_test {
/*
    public static $includecoverage = array('lib/portfoliolib.php', 'mod/partforum/lib.php');
    public $module_type = 'partforum';
    ...
    public function setUp() {
        ...
        $discussions = $DB->get_records('partforum_discussions', array('partforum' => $first_module->id));
        $first_discussion = reset($discussions);

        $posts = $DB->get_records('partforum_posts', array('discussion' => $first_discussion->id));
        $first_post = reset($posts);

        $callbackargs = array('postid' => $first_post->id, 'discussionid' => $first_discussion->id);
        $this->postcaller = parent::setup_caller('partforum_portfolio_caller', $callbackargs, $first_post->userid);

        unset($callbackargs['postid']);
        $this->discussioncaller = parent::setup_caller('partforum_portfolio_caller', $callbackargs, $first_post->userid);
